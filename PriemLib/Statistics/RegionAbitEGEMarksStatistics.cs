    ...
    public partial class RegionAbitEGEMarksStatistics : Form
    {
        ...
        private void FillComboRegion()
        {
            string query = @"
                ...
                INNER JOIN ed.Region ON Region.Id = extPerson.RegionId
            ...
            List<KeyValuePair<string, string>> bind =
                (from DataRow rw in tbl.Rows
                 select new
                     KeyValuePair<string, string>(rw["Id"].ToString(), rw["Name"].ToString())
                ).Distinct().ToList();
        ...
        private void FillComboEgeExamName()
        {
            ...
            string query = @"
                ...
                INNER JOIN ed.Entry ON Entry.Id = Abiturient.EntryId
                INNER JOIN ed.StudyLevel ON StudyLevel.Id = Entry.StudyLevelId
                WHERE StudyLevel.LevelGroupId = @LevelGroupId AND extPerson.RegionId=@RegionId ";
        ...
        private void FillGrid()
        {
            string query = @"
                ...
                INNER JOIN ed.extPerson ON extPerson.Id = hlpStatMaxApprovedEgeMarks.PersonId
                INNER JOIN ed.EgeExamName ON EgeExamName.Id = hlpStatMaxApprovedEgeMarks.EgeExamNameId
                INNER JOIN ed.Abiturient ON Abiturient.PersonId = extPerson.Id
                INNER JOIN ed.Entry ON Entry.Id = Abiturient.EntryId
