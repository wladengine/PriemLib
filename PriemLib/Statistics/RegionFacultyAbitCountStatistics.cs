...
(convert(float, t.CNT_Reg)/convert(float, t.CNT_All))*convert(float, 100) AS '% от общ', 
t.MinEGE AS 'Мин сумма ЕГЭ', t.AvgEGE AS 'Сред сумма ЕГЭ', t.MaxEGE AS 'Макс сумма ЕГЭ'
FROM 
(
SELECT DISTINCT FacultyName, COUNT(DISTINCT extPerson.Id) AS 'CNT_Reg', 
...
WHERE qAbitAll.StudyLevelGroupId=@StudyLevelGroupId AND extPerson.RegionId=@RegionId
";
