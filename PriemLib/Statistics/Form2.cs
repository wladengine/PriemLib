...
INNER JOIN ed.extEntryView ON extEntryView.AbiturientId=qAbiturient.Id 
INNER JOIN ed.extPerson ON extPerson.Id=qAbiturient.PersonId 
WHERE qAbiturient.StudyLevelGroupId=1 AND qAbiturient.StudyFormId='1' AND qAbiturient.CompetitionId IN (2,7) AND extPerson.RegionId=1 
GROUP BY qAbiturient.StudyBasisId";
            tbl = MainClass.Bdc.GetDataSet(query).Tables[0];
            ...
            query = @"SELECT qAbiturient.StudyBasisId, COUNT(DISTINCT qAbiturient.PersonId) AS CNT 
FROM ed.qAbiturient 
INNER JOIN ed.extEntryView ON extEntryView.AbiturientId=qAbiturient.Id 
